...
def plot_embedding_2D(data, label, title):
    ...
    l = ['AMD',"DME","NORMAL"]
    color = [plt.cm.Set1(0), plt.cm.Set1(1), plt.cm.Set1(2)]
    patches = [mpatches.Patch(color=color[i], label='{:s}'.format(l[i])) for i in range(3)]
    plt.legend(handles=patches)

    plt.title(title)
    plt.savefig('./distributionB.jpg', dpi=500)
    return fig
...
def main():
    ...
    result_2D = tsne_2D.fit_transform(BB)
    print('Finished......')

    fig1 = plot_embedding_2D(result_2D, B_label,'t-sne')
    plt.show()

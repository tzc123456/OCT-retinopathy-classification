...
def get_parser():
    ...
    parser.add_argument('--data_dir', type=str,  default='/home/xiaoxie/data/tzc/domain_adaptation')
    parser.add_argument('--src_domain', type=str, default="A")
    parser.add_argument('--tgt_domain', type=str, default="C")
...
def get_model(args):
    ...
    #model = nn.DataParallel(model,device_ids= [0,1,2,3]).to(args.device)
    # .to(args.device)
    #model = model.cuda()
    return model
...
def train(source_loader, target_train_loader, target_test_loader, model, optimizer, lr_scheduler, args):
    ...
    for e in range(1, args.n_epoch+1):
        ...
        for _ in range(n_batch):
            ...
            clf_loss, transfer_loss = model(data_source, data_target, label_source,e)
        ...
        if test_acc > best_test_acc:
            ...
            torch.save(model.state_dict(), './ProposedAC.pth')
            print('save model')
            print(best_test_acc)
...
def main():
    print("DSAN idea2____________----AC---")
    parser = get_parser()

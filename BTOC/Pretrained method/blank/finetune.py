...
parser.add_argument('--src', type=str, default='C')
parser.add_argument('--tar', type=str, default='A')
parser.add_argument('--n_class', type=int, default=3)
parser.add_argument('--lr', type=float, default=1e-4) # 1e-4
parser.add_argument('--n_epoch', type=int, default=100)
parser.add_argument('--momentum', type=float, default=0.9)
parser.add_argument('--decay', type=float, default=5e-4)
parser.add_argument('--data', type=str, default=r'/home/xiaoxie/data/tzc/domain_adaptation/DA/data')
...
class DANNmodel(nn.Module):
    def __init__(self):
        ...
        model.load_state_dict(torch.load(
            r'/home/xiaoxie/data/tzc/domain_adaptation/DA/BTOC/Proposed/ProposedBC.pth'))
...
def finetune(model, dataloaders, optimizer):
    ...
    for epoch in range(1, args.n_epoch + 1):
        ...
        for phase in ['src', 'val']:
            ...
            # else:
            #     model.eval()
                train_total_loss, train_correct = 0, 0
                ...
                epoch_loss = train_total_loss / len(dataloaders[phase].dataset)
                epoch_acc = train_correct.double() / len(dataloaders[phase].dataset)
                writer.add_scalar("Train Loss/Epoch", epoch_loss, epoch)
                writer.add_scalar("Train Acc/Epoch", epoch_acc, epoch)
                ...
                epoch_loss = val_total_loss / len(dataloaders[phase].dataset)
                epoch_acc = val_correct.double() / len(dataloaders[phase].dataset)
                writer.add_scalar("Val Loss/Epoch", epoch_loss, epoch)
                writer.add_scalar("Val Acc/Epoch", epoch_acc, epoch)
                print('Epoch: [{:02d}/{:02d}]---{}, loss: {:.6f}, acc: {:.4f}'.format(epoch, args.n_epoch, phase,
                                                                                      epoch_loss,
                                                                                      epoch_acc))
                ...
                epoch_loss = test_total_loss / len(dataloaders[phase].dataset)
                epoch_acc = test_correct.double() / len(dataloaders[phase].dataset)
                writer.add_scalar("Test Acc/Epoch", epoch_acc, epoch)
                print('Epoch: [{:02d}/{:02d}]---{}, loss: {:.6f}, acc: {:.4f}'.format(epoch, args.n_epoch, phase,
                                                                                      epoch_loss,
                                                                                      epoch_acc))
            ...
            if phase == 'val' and epoch_acc > best_acc:
                stop = 0
                best_acc = epoch_acc
                torch.save(model.state_dict(), 'DA-model.pkl')
        # if stop >= args.early_stop:
        #     break
        print()
    print("best val acc:{}".format(best_acc))
    model.load_state_dict(torch.load('DA-model.pkl'))
